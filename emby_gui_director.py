...
from emby_gui import APP_TITLE, save_director_backup


class DirectorTabMixin:
    def build_director_tab(self) -> None:
        controls = self.top_controls(self.director_tab, self.director_lib_var)
    ...
    def scan_directors(self) -> None:
        try:
            client = self.client()
            libraries = self.libraries(self.director_lib_var.get())
        ...
        def worker() -> list[dict[str, Any]]:
            ...
            for library_id in libraries:
                for movie in client.query_items(library_id, include_item_types="Movie", fields="People,Path"):
        ...
        def done(rows: list[dict[str, Any]]) -> None:
            ...
            for row in rows:
                self.director_tree.insert(
                    ...
                    "end",
                    iid=row["Id"],
                    values=(row["Name"], row["Id"], ", ".join(row["Directors"]), row["Path"], row["Status"]),
                )

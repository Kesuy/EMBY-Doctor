...
from emby_gui import APP_TITLE, export_missing_csv


class MissingActorsTabMixin:
    def build_missing_tab(self) -> None:
        controls = self.top_controls(self.missing_tab, self.missing_lib_var)
    ...
    def scan_missing_actors(self) -> None:
        try:
            client = self.client()
            libraries = self.libraries(self.missing_lib_var.get())
        ...
        def done(rows: list[dict[str, Any]]) -> None:
            ...
            for i, row in enumerate(rows):
                ...
                self.missing_tree.insert(
                    ...
                    "end",
                    iid=iid,
                    values=(row["Name"], row["Id"], row["LibraryId"], row["Path"]),
                )
